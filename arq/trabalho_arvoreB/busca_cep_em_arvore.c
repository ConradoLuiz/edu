...
int main(int argc, char** agrv){
    ...
    if(argc != 2){
        printf("Digitar apenas o nome do programa e o CEP para pesquisar!!\n");
        return 0;
    }

    char* cep_pesquisado = agrv[1];

    ArvoreB* arvore = ArvoreB_Abre("arvore.dat");

    long posicao = ArvoreB_Busca(arvore, cep_pesquisado);
    ...
    if (posicao != -1){
        ...
        if (!f){
            printf("O cep foi encontrado na estrutura, porem nao foi possivel abrir o arquivo cep.dat\n");
            return 1;
        }
        
        Endereco e;
        fseek(f, posicao - (posicao % sizeof(Endereco)), SEEK_SET);
