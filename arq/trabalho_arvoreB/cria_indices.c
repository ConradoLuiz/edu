...
int main(){
    ...
    FILE *f;
    Endereco e;
    long tamanho_cep, posicao;
    int qt = 1;
    f = fopen("cep.dat","r");
    if (!f){
        printf("Nao foi possivel ler o arquivo cep.dat\n");
        return 1;
    }
    fseek(f, 0, SEEK_END);
    tamanho_cep = ftell(f)/sizeof(Endereco);
    ...
    while(qt != 0){
        posicao = ftell(f); // pegando a posicao do fseek

        qt = fread(&e, sizeof(Endereco), 1, f); // lendo do arquivo cep.dat

        sprintf(chave, "%s", e.cep);
        ArvoreB_Insere(arvore, chave, posicao);
